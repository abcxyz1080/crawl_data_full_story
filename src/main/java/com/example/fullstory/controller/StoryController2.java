...
public class StoryController2 {
	...
	public String createStory() throws IOException {
		...
		for (Element cate : categories) {
			while (true) {
				Document doc_cate = Jsoup.connect(cate.attr("href") + "trang-" + page).get();
				stories = doc_cate.select("div.col-xs-7 > div > h3 > a");
				Elements authors = doc_cate.select("div.col-xs-7 > div > span.author");

				Document doc_cate_2 = Jsoup.connect(cate.attr("href") + "trang-" + (page + 1)).get();
				...
				for (int i = 0; i < stories.size(); i++) {
					...
					checkDuplicated = storyRepository.findAll().stream()
							.anyMatch((s) -> s.getTitle().equals(nameStory));
					if (!checkDuplicated) {
